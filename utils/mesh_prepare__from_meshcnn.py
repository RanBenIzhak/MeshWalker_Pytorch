...
def fill_mesh(mesh2fill, file: str, opt, loaded_mesh=None):
    load_path = get_mesh_path(file, opt.num_aug)
    if 0:#os.path.exists(load_path):
        mesh_data = np.load(load_path, encoding='latin1', allow_pickle=True)
    else:
        mesh_data = from_scratch(file, opt, loaded_mesh=loaded_mesh)
...
def calc_edge_features(mesh):
    mesh_data = from_scratch(loaded_mesh=mesh)
    features = mesh_data.features
    edges = mesh_data.edges

    return edges, features

def from_scratch(file='', opt=None, loaded_mesh=None):
    ...
    if loaded_mesh is not None:
        mesh_data.vs, faces = loaded_mesh.vertices, loaded_mesh.faces
    else:
...
def check_features():
    file = r'C:\datasets\ModelNet40\person\train\person_0004.off'
    file = r'C:\datasets\ModelNet40\tv_stand\train\tv_stand_0006.off'
    mesh = trimesh.load_mesh(file)
    edges, features = calc_edge_features(mesh)
    features_ = features.T

    open3d_mesh = open3d.geometry.TriangleMesh()
    open3d_mesh.vertices = open3d.utility.Vector3dVector(mesh.vertices)
    open3d_mesh.triangles = open3d.utility.Vector3iVector(mesh.faces)
    vertex_colors = np.zeros((mesh.vertices.shape[0], 3))
    ftr2use = features_ / features_.max(axis=0)
    for i in range(edges.shape[0]):
        e = edges[i]
        f = ftr2use[i]
        vertex_colors[e[0], :] = f[:3]*2
        vertex_colors[e[1], :] = f[:3]*2
    open3d_mesh.vertex_colors = open3d.utility.Vector3dVector(vertex_colors.astype('uint8'))
    open3d.visualization.draw_geometries([open3d_mesh])
